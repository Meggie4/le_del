...
clock_gettime(CLOCK_MONOTONIC,&begin); //begin insert , so begin counting 
	...
	strcpy(recorder_name,"record");
	//recorder_name=FLAGS_db+7;
	printf("db_bench.cc, FLAGS_db=%s, recorder_name=%s\n",FLAGS_db, recorder_name);
	//exit(9);
    ...
    for (i = 0; i < num_; i += entries_per_batch_) {
	  ...
	  switch(sync_ratio){
		...
		default:
			rand=random()%100;
			if(rand<sync_ratio) sync_flag=true;
			break;
	  
	  }
	  //printf("rand=%d,sync=%d,Flags_ratio=%d\n",rand, sync_flag,Flags_ratio);
	  write_options_.sync=sync_flag;
      s = db_->Write(write_options_, &batch);
      if (!s.ok()) {
        fprintf(stderr, " I am db_bench.cc,  do write, put error: %s\n", s.ToString().c_str());
